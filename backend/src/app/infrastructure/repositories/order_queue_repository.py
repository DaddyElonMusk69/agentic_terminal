        ...
        async with self._sessionmaker() as session:
            async with session.begin():
                result = await session.execute(
                    select(OrderQueueRequestModel)
                    .where(OrderQueueRequestModel.status == "queued")
                    .order_by(OrderQueueRequestModel.created_at)
    ...
    async def _finalize(
        ...
        async with self._sessionmaker() as session:
            await session.execute(
                update(OrderQueueRequestModel)
                .where(OrderQueueRequestModel.id == request_id)
                .values(
                    status=status,
                    result=result,
                    error=error,
