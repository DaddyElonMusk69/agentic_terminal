        ...
        async with self._sessionmaker() as session:
            async with session.begin():
                result = await session.execute(
                    select(PromptBuildRequestModel)
                    .where(PromptBuildRequestModel.status == "queued")
                    .order_by(PromptBuildRequestModel.created_at)
    ...
    async def _finalize(
        ...
        async with self._sessionmaker() as session:
            await session.execute(
                update(PromptBuildRequestModel)
                .where(PromptBuildRequestModel.id == request_id)
                .values(
                    status=status,
                    result=result,
                    error=error,
